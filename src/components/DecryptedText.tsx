...
interface DecryptedTextProps extends HTMLMotionProps<'span'> {
  ...
  animateOn?: 'view' | 'hover' | 'inViewHover' | 'click' | 'theme';
...
export default function DecryptedText({
  ...
  encryptedClassName = '',
  animateOn = 'hover',
  themeClassName = 'random-scroll-dark',
  clickMode = 'once',
  ...props
}: DecryptedTextProps) {
  ...
  useEffect(() => {
    ...
    const finishCycle = () => {
      ...
      if (animateOn === 'theme' && document.documentElement.classList.contains(themeClassName)) {
  ...
  useEffect(() => {
    if (animateOn !== 'theme') return;

    const root = document.documentElement;
    let wasDark = root.classList.contains(themeClassName);
    const observer = new MutationObserver(() => {
      const isDark = root.classList.contains(themeClassName);
      if (isDark && !wasDark) {
    ...
    if (wasDark) triggerDecrypt();
    return () => {
      observer.disconnect();
      if (loopTimeoutRef.current) clearTimeout(loopTimeoutRef.current);
    };
  }, [animateOn, resetToPlainText, themeClassName, triggerDecrypt]);
